...
### 5.4 admin 페이지 구조 (사용자 캡처로 확인됨, 2026-05-21)

**URL**: `https://admin.kubecha.com/brew/galaxy/movies`

**검색 동작**:
- 페이지 상단에 "Search for title" 텍스트 입력 필드와 "Search" 버튼이 있음
- 입력 후 "Search" 클릭 → 결과 테이블이 같은 페이지 내에 갱신됨

**결과 테이블 컬럼** (왼쪽부터):
| 컬럼 | 추출 대상 | 메모 |
|---|---|---|
| id | ✅ 추출 | 숫자 (예: 1588566) |
| code | ✅ 추출 | 영숫자 문자열 (예: m5Nnk08) |
| poster | — | 이미지, 무시 |
| title | ✅ 동명이작 매칭에 사용 | 한글 제목 + 언어 태그(`ko`) 함께 표시 |
| year | ✅ 동명이작 매칭에 사용 | 4자리 연도 (KOBIS 개봉연도와 비교) |
| genre | — | 보조 정보 |
| directors | — | 보조 정보 |
| nations | — | 보조 정보 |
| sod_status / tod_status / links | — | 무시 |

**핵심 이점**: id와 code가 결과 테이블에 직접 노출되므로 **상세 페이지 진입 불필요**. 검색 1회로 매칭 가능.

**로그인 URL**: `https://admin.kubecha.com/brew/session/new` (Rails 스타일 세션 페이지)

**로그인 폼 구조** (사용자 캡처로 확인됨, 2026-05-21):
- 페이지 제목: "login"
- 필드 1: 라벨 "email:" + 텍스트 입력 (첫 번째 `<input>`)
- 필드 2: 라벨 "password:" + 입력 (두 번째 `<input>`, `type="password"` 추정)
- 제출 버튼: 텍스트 "제출"

**Playwright 셀렉터 전략 (다중 fallback)**:
- email 입력: `input[type="email"]` → `input[name*="email"]` → 첫 번째 `<input>`
- password 입력: `input[type="password"]` → 두 번째 `<input>`
- 제출 버튼: `button:has-text("제출")` → `input[type="submit"]` → `input[value="제출"]`

**로그인 후 흐름**: `https://admin.kubecha.com/brew/galaxy/movies`로 직접 이동하여 검색

**Playwright 셀렉터 전략**:
- 텍스트/라벨 기반 우선: `placeholder="Search for title"`, `text="Search"`
- 결과 테이블: `<table>` 내의 `<tr>` 순회, 각 `<td>`에서 id/code/title/year 추출
- CSS 셀렉터는 admin이 개편되더라도 텍스트 기반보다 안정적인 위치만 사용
...
- **admin 영화 검색 페이지**: ✅ 확인 완료 (2026-05-21)
- **admin 로그인 페이지**: ✅ 확인 완료 (2026-05-21)
